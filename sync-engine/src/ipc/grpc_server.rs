...
use crate::models::sync_job::{SyncJob, SyncJobConfig, SyncDirection as JobSyncDirection, SyncStatus};
...
impl SyncEngine for SyncEngineServer {
    ...
    async fn create_job(
        ...
        let config = SyncJobConfig {
            ...
            direction: match req.direction() {
                SyncDirection::Bidirectional => JobSyncDirection::Bidirectional,
                SyncDirection::SourceToTarget => JobSyncDirection::SourceToTarget,
                SyncDirection::TargetToSource => JobSyncDirection::TargetToSource,
            },
            conflict_strategy: match req.conflict_strategy() {
                ConflictStrategy::LastWriteWins => crate::models::sync_job::ConflictStrategy::LastWriteWins,
                ConflictStrategy::Manual => crate::models::sync_job::ConflictStrategy::Manual,
                ConflictStrategy::Skip => crate::models::sync_job::ConflictStrategy::Skip,
            },
    ...
    async fn job_status(
        ...
        let status = match job.status {
            SyncStatus::Idle => JobStatus::Idle,
            SyncStatus::Scanning => JobStatus::Scanning,
            SyncStatus::Comparing => JobStatus::Comparing,
            SyncStatus::Transferring => JobStatus::Transferring,
            SyncStatus::ResolvingConflicts => JobStatus::ResolvingConflicts,
            SyncStatus::Completed => JobStatus::Completed,
            SyncStatus::Failed => JobStatus::Failed,
            SyncStatus::Paused => JobStatus::Paused,
        };

        let progress = Some(SyncProgress {
        ...
        Ok(Response::new(JobStatusResponse {
            ...
            start_time: job.start_time.map(|t| t.timestamp()).unwrap_or(0),
            end_time: job.end_time.map(|t| t.timestamp()).unwrap_or(0),
        }))
    }
...
pub async fn start_grpc_server(
    ...
    let sync_engine_server = SyncEngineServer::new(orchestrator);

    let addr = addr.parse()?;
    
    Server::builder()
        .add_service(sync_engine_server::SyncEngineServer::new(sync_engine_server))
        .serve(addr)
        .await?;
